...
class Iceberg(QWidget):
    ...
    def __init__(self):
        ...
        for fila, p in enumerate(self.platforms):
            self.tabla.setItem(fila, 0, QTableWidgetItem(p["name"]))

            self.tabla.setItem(fila, 1, QTableWidgetItem(str(p["lat"])))

            self.tabla.setItem(fila, 2, QTableWidgetItem(str(p["lon"])))

            self.tabla.setItem(fila, 3, QTableWidgetItem(str(p["depth"])))
    ...
    def miles_to_decimal(self, degree, minutes, seconds, posicion):
        ...
        decimal_posicion = degree + minutes / 60 + seconds / 3600

        return -decimal_posicion if posicion in ["S", "W"] else decimal_posicion

    def calcular_riesgos(self, lat0, lon0, heading, keel_depth):
        ...
        def distance_nm(lat1, lon1, lat2, lon2):
            ...
            dlon = (lon1 - lon2) * 60 * math.cos(math.radians(lat2))
        ...
        for _ in range(max_steps):
            lat += (step_nm / 60) * math.cos(theta)

            lon += (step_nm / (60 * math.cos(math.radians(lat)))) * math.sin(theta)

            trayectoria.append((lat, lon))

            for p in self.platforms:
                distance = distance_nm(lat, lon, p["lat"], p["lon"])
            ...
            resultados.append(
                {
                    ...
                    "name": p["name"],
                    "min_dist": p["min_dist"],
                    "surface": surface_risk(keel_depth, p["min_dist"], p["depth"]),
                    "subsea": subsea_risk(keel_depth, p["depth"], p["min_dist"]),
                }
            )
    ...
    def show_results(self):
        ...
        try:
            ...
            keel_depth = float(self.keel_input.text())

        except Exception as e:
            print("Error:", e)
            return

        resultados, trayectoria = self.calcular_riesgos(lat0, lon0, heading, keel_depth)

        # llenar tabla
        for fila, r in enumerate(resultados):
            self.tabla.setItem(fila, 4, QTableWidgetItem(str(round(r["min_dist"], 2))))

            self.tabla.setItem(fila, 5, QTableWidgetItem(r["surface"]))

            self.tabla.setItem(fila, 6, QTableWidgetItem(r["subsea"]))
        ...
        self.graph.plot(lons, lats, pen="b")
