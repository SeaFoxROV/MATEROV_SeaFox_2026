...
class BoundingBox:
    ...
    def __init__(
        self, x1: int, y1: int, x2: int, y2: int, box_id: Optional[int] = None
    ):
        if box_id is None:
...
class BoxCanvas(QLabel):
    ...
    def _to_w(self, ix: int, iy: int):
        ...
        return int(self._img_rect.x() + ix * sx), int(self._img_rect.y() + iy * sy)
    ...
    def mouseMoveEvent(self, e):
        ...
        if self._mode == _Mode.DRAW and self._drag_start:
            ...
            self._new_rect = QRect(min(x0, wx), min(y0, wy), abs(wx - x0), abs(wy - y0))
            self.update()
    ...
    def update_boxes(self, boxes: List[BoundingBox], validated: bool):
        ...
        if validated:
            ...
            self.lbl_note.setText(
                "pending validation" if boxes else "awaiting detection"
            )
            self.lbl_note.setStyleSheet(f"color:{_MUTED}; font-size:11px;")
...
class StatusBar(QLabel):
    def __init__(self, parent=None):
        ...
        self._base = (
            f"background:{_BG}; font-size:11px;"
    ...
    def post(self, msg: str, level: str = "info"):
        c = {"info": _MUTED, "ok": _ACCENT, "warn": "#c08030", "err": "#c04040"}.get(
            level, _MUTED
        )
...
class crabVision(QWidget):
    ...
    def receive_detection(self, payload: dict):
        ...
        self.status.post(
            f"received {self._pilot_box_count} detections — review and validate", "info"
        )
